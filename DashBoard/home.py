...
def run_home():
  ...
  st.markdown("## 대시보드 개요 \n" 
             "이 대시보드는 서울시 부동산 데이터를 활용하여 부동산 시장의 트렌드를 분석하고 시각화한 대시보드입니다. \n"
              "아래는 데이터의 개요입니다.")
  total_df['DEAL_YMD'] = pd.to_datetime(total_df['DEAL_YMD'], format='%Y-%m-%d')    # 날짜 형식으로 변환   
  total_df['month'] = total_df['DEAL_YMD'].dt.month                                 # 월 컬럼 추가
  total_df = total_df.loc[total_df['HOUSE_TYPE'] == '아파트', :]                    # 아파트만 선택

  sgg_nm = st.sidebar.selectbox('자치구 선택', (total_df['SGG_NM'].unique()))         # 자치구 선택
  selected_month = st.sidebar.radio("확인하고 싶은 월을 선택하세요", ['12월', '1월']) # 월 선택
  month_dict = {'12월': 12, '1월': 1}                                                 # 월 딕셔너리
  
  st.markdown("<hr>", unsafe_allow_html=True)                                         # 구분선
  st.subheader(f'{sgg_nm} 자치구 {selected_month} 아파트 가격 개요')                  # 선택한 자치구, 월에 따른 아파트 가격 개요
  st.markdown("자치구와 월을 클릭하면 각 지역구의 거래된 **최소가격**, **최대가격**을 확인할 수 있습니다.")
  
  col1, col2 = st.columns(2)                                                    # 2개의 컬럼 생성
  filtered_month = total_df[total_df['month'] == month_dict[selected_month]]    # 선택한 월에 해당하는 데이터만 필터링
  filtered_month = filtered_month[filtered_month['SGG_NM'] == sgg_nm]           # 선택한 자치구에 해당하는 데이터만 필터링
  march_min_price = filtered_month['OBJ_AMT'].min()                             # 최소가격
  march_max_price = filtered_month['OBJ_AMT'].max()                             # 최대가격
  ...
  sorted_df = filtered_month[['SGG_NM', 'BJDONG_NM', 'BLDG_NM', 'BLDG_AREA', 'OBJ_AMT']]            # 필요한 컬럼만 선택
