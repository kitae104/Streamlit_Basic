...
# 라디오 선택 버튼
mbti = st.radio("당신의 MBTI는 무엇인가요?", ("ISTJ", "ENFP", "잘 모르겠어요"))
...
# 선택 박스
mbti = st.selectbox(
    "당신의 MBTI를 선택해 주세요", ("ISTJ", "ENFP", "잘 모르겠어요"), index=2
)
...
# 슬라이더
values = st.slider(
    "오늘의 기분을 0에서 100으로 표현해 보세요! :sparkles:", 0.0, 100.0, (30.0, 80.0)
)
...
# 텍스트 입력
title = st.text_input(
    label="방학 때 가고 싶은 장소가 있나요?", placeholder="장소를 입력해 주세요!"
)
st.write(f"당신이 가고 싶은 장소는 :violet[{title}]입니다!")

# 숫자 입력
number = st.number_input(
    label="당신의 나이는 몇 살인가요?", min_value=10, max_value=100, value=15, step=1
)
